...
{
	//double ts = static_cast<double>(timestamp) / static_cast<double>(1e6);
	//cout << "Inside lidar sense " << ts << endl;
...
{
	//double ts = static_cast<double>(timestamp) / static_cast<double>(1e6);
	//cout << "Inside radar sense " << ts << endl;
...
void Tools::ukfResults(Car car, pcl::visualization::PCLVisualizer::Ptr& viewer, double time, int steps)
{
	...
	//cout << "Inside ukf result" << endl;
	//cout << time << "  "<< steps << endl;
	UKF ukf = car.ukf;
	...
	if(time > 0)
	{
		double dt = time/steps; // 2/6 0.33 < 2 
		double ct = dt;
		while(ct <= time)
		{
			ukf.Prediction(dt);
